...
def _build_para_margin_linespacing(pp, ps):
    ...
    # Case: half values (HwpUnitChar)
    margin_case = sub(case, "hh", "margin")
    sub(margin_case, "hc", "intent", {"value": str(indent), "unit": "HWPUNIT"})
    sub(margin_case, "hc", "left", {"value": str(margin_left), "unit": "HWPUNIT"})
    sub(margin_case, "hc", "right", {"value": str(margin_right), "unit": "HWPUNIT"})
    sub(margin_case, "hc", "prev", {"value": str(margin_top), "unit": "HWPUNIT"})
    sub(margin_case, "hc", "next", {"value": str(margin_bottom), "unit": "HWPUNIT"})
    ls_case = sub(case, "hh", "lineSpacing")
    ls_case.set("type", ls_type_str)
    ls_case.set("value", str(linespacing))
    ls_case.set("unit", "HWPUNIT")

    # Default: doubled values
    default = sub(switch, "hp", "default")
    margin_def = sub(default, "hh", "margin")
    sub(margin_def, "hc", "intent", {"value": str(indent * 2), "unit": "HWPUNIT"})
    sub(margin_def, "hc", "left", {"value": str(margin_left * 2), "unit": "HWPUNIT"})
    sub(margin_def, "hc", "right", {"value": str(margin_right * 2), "unit": "HWPUNIT"})
    sub(margin_def, "hc", "prev", {"value": str(margin_top * 2), "unit": "HWPUNIT"})
    sub(margin_def, "hc", "next", {"value": str(margin_bottom * 2), "unit": "HWPUNIT"})
